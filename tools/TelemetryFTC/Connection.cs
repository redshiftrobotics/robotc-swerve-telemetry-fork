...
namespace TelemetryFTC
    ...
    public class KnownNXT
        ...
        // for use in listbox UI
            ...
            //
            default: return Program.Fail<string>(); 
                }
            }}   
        ...
        Connection NewConnection()
            ...
            //
            default: return Program.Fail<Connection>(); 
                }
            }
        ...
        public void SendJoystickMessage()
            {
            this.connection.SendJoystickMessage();
            }
        public void SendTelemetryPollMessage()
            {
            this.connection.SendTelemetryPollMessage();
            }
        public void QueryAvailableNXTData()
            {
            this.connection.QueryAvailableNXTData();
            }
    ...
    public abstract class Connection : WIN32, IDisposable
        ...
        public virtual bool ProbeForNXT(out string sNxtName)
            ...
            Program.Trace("opening {0}...", this);
            if (this.Open(false))
                {
                Program.Trace("...opened {0}", this);
                //
                try {
                ...
                {
                Program.Trace("...failed to open {0}", this);
                }
        ...
        // Find the message that we sent that that's a reply to.
            ...
            // Messages that were sent a very long time ago we should assume
            // are not ever going to see a reply
            System.TimeSpan stalenessInterval = System.TimeSpan.FromMilliseconds(1000);
            ...
            lock (this.msgReplyTargets)
                ...
                for (int imsg = 0; imsg < this.msgReplyTargets.Count; )
                    ...
                    if (this.msgReplyTargets[imsg].TimeOfTransmission + stalenessInterval < System.DateTime.Now)
                        {
    ...
    class IPConnection : Connection
        ...
        bool AcquireExclusiveUse()
            ...
            if (IsLocalIPAddress(sResp))
                ...
                fResult = true;
                Program.Trace("exclusive use of {0} acquired", this.epSamanthaTCP);
                }
            else
                {
                // Ask for more info as to what the problem is
                string sError = SendWebRequest(this.epHTTP, "/samantha/SAMANTHALastErrorByIP.htm");
                Program.ReportError("Unable to connect to Samantha module {0}: '{1}'", this.epSamanthaTCP.Address.ToString(), sError);
                }
            ...
            try {
                if (this.AcquireExclusiveUse())
                    ...
                    this.SocketMonitorLock(() =>
                        ...
                        Program.Trace("IP: opened: {0}", this.epSamanthaTCP);
                        });
        ...
        //  Close down resources we have acquired
            ...
            this.SocketMonitorLock(() => 
                ...
                if (this.socket != null)
                    ...
                    this.socket.Close();
                    this.socket = null;
                    Program.Trace("socket {0} closed", this.epSamanthaTCP);
                    }
                });
            ...
            if (this.fHaveExclusive)
                ...
                this.fHaveExclusive = false;
                Program.Trace("exclusive use of {0} released", this.epSamanthaTCP);
                }
            }
